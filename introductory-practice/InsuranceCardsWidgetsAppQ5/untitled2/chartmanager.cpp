...
void ChartManager::updatePieChart(QTableWidget *tableWidget)
{
    if (!tableWidget || !m_pieSeries) {
        qWarning() << "ChartManager::updatePieChart called with null tableWidget or uninitialized pieSeries";
        return;
    }

    qDebug() << "Updating Pie Chart data...";
    ...
    qDebug() << "Collected data:" << insuranceCounts << "Total:" << totalCount;
    ...
    if (totalCount > 0) {
        for (int i = 0; i < insuranceTypes.size(); ++i) {
            ...
            if (insuranceCounts.contains(type) && insuranceCounts[type] > 0) {
                ...
                QtCharts::QPieSlice *slice = m_pieSeries->append(type, count);
                ...
                connect(slice, &QtCharts::QPieSlice::hovered, this, [slice, type, count](bool hovered) {
                    if (!slice) return;
        ...
        if (m_chartTitleLabel && m_chartTitleLabel->text() != "Распределение видов страхования") {
         ...
         QtCharts::QPieSlice* emptySlice = m_pieSeries->append("Нет данных", 1);
        emptySlice->setLabelVisible(false);
        if (m_chartTitleLabel) m_chartTitleLabel->setText("Виды страхования (Нет данных)");
    }
    qDebug() << "Pie Chart data updated. Slice count:" << m_pieSeries->count();

    if (!m_chartView) {
        qDebug() << "Initializing Chart View and related widgets...";
        ...
        qDebug() << "Chart View and related widgets initialized.";
    }
}
