                          ...
                          const QMap<QString, int> &monthCounts)
{
    ...
    for (int i = 0; i < sortedAgents.size(); ++i) {
        ...
        if (pair.second > maxAgentSum) maxAgentSum = pair.second;
    }
    qDebug() << "Agent line series created with" << agentSeries->count() << "points. Max sum:" << maxAgentSum;

    QPen agentPen(agentLineColor);
    agentPen.setWidth(2);
    agentSeries->setPen(agentPen);
    agentSeries->setPointsVisible(true);
//    agentSeries->setMarkerSize(8.0);
    ...
    if (!agentCategories.isEmpty()) axisXAgents->append(agentCategories);
    ...
    if (agentCategories.count() > 15) axisXAgents->setLabelsAngle(-60); else axisXAgents->setLabelsAngle(0);
    ...
    int tickCountYAgent = 5; double niceMaxYAgent = 1000;
    if (maxAgentSum > 0) {
        double step = qPow(10.0, qFloor(qLn(maxAgentSum) / qLn(10.0)));
        if (maxAgentSum / step < 2) step /= 5.0; else if (maxAgentSum / step < 5) step /= 2.0;
        if (step <= 0) step = 100;
                ...
                if (index >= 0 && index < agentCategories.size()) {
                    ...
                    if (state) {
                        QToolTip::showText(QCursor::pos(), QString("%1\nСумма: %L2 ₽")
                                                               .arg(agentName).arg(value, 0, 'f', 0), nullptr);
                    } else {
                        QToolTip::hideText();
                    }
                } else {
    ...
    qDebug() << "Agent Line Chart created and set.";
    qDebug() << "Creating Month Vertical Bar Chart...";

    QMap<QDate, int> sortedMonths;
    for (auto it = monthCounts.constBegin(); it != monthCounts.constEnd(); ++it) {
        if (it.value() > 0) sortedMonths[QDate::fromString(it.key() + "-01", "yyyy-MM-dd")] = it.value();
    }
    qDebug() << "Month data sorted:" << sortedMonths.size() << "months";
    ...
    for (auto it = sortedMonths.constBegin(); it != sortedMonths.constEnd(); ++it) {
        ...
        *monthSet << value;
        ...
        if (value > maxMonthValue) maxMonthValue = value;
    }
    qDebug() << "Month vertical bar series created with" << monthSet->count() << "bars. Max value:" << maxMonthValue;
    ...
    if (!monthCategories.isEmpty()) axisXMonths->append(monthCategories);
    ...
    int tickCountY = 5; double niceMaxY = 10;
    if (maxMonthValue > 0) {
        double step = qPow(10.0, qFloor(qLn(maxMonthValue) / qLn(10.0)));
        if (maxMonthValue / step < 2) step /= 5.0; else if (maxMonthValue / step < 5) step /= 2.0;
        if (step <= 0) step = 1;
    ...
    connect(monthSeries, &QBarSeries::hovered, this,
            [monthSet, monthOriginalColor](bool status, int index, QBarSet *barset) {
                if (!barset || barset != monthSet) return;
                if (status) {
    ...
    qDebug() << "Month Vertical Bar Chart created and set.";
    qDebug() << "ChartDialog::setData finished.";
}
